...
function createClient(bin, config) {
  ...
  const clientOptions = {
    ...
    middleware: {
      ...
      async provideDiagnostics(document2, previousResultId, token, next) {
        const bufnr = import_coc.workspace.getDocument(document2.uri).bufnr;
        const buf = import_coc.diagnosticManager.getItem(bufnr);
        const currentDiagnostics = import_coc.diagnosticManager.getDiagnostics(buf)["rust-analyzer"] ?? [];
        const rustcDiagnosticItems = currentDiagnostics.filter((d) => d.source === "rustc");
        const raDiagnostics = await next(document2, previousResultId, token);
        const diagnostics = (raDiagnostics == null ? void 0 : raDiagnostics.kind) === import_coc.DocumentDiagnosticReportKind.Full ? raDiagnostics : {
          kind: import_coc.DocumentDiagnosticReportKind.Full,
          items: []
        };
